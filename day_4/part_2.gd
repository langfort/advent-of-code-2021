...
func _output_result() -> void:
	print("Day 4 Part 2 result: %d" % result)
	...
	for number in call_numbers:
		if number_map.has(number):
			for entry in number_map[number]:
				...
				var board_number: int = floor(entry.y / 5)
				if _check_win(entry) and not winning_boards.has(board_number):
					winning_boards.push_back(board_number)
					if winning_boards.size() == data.size() / 5:
